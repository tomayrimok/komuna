import { Box, Flex, For, Image, SkeletonText, Stack, Text } from '@chakra-ui/react';
...
const Expenses = () => {
  ...
  const dataPerMonth = data?.apartmentExpenses
    ? groupBy(data.apartmentExpenses, (item) => format(parseISO(item.createdAt), 'M/yyyy'))
    : {};
            ...
            {([month, expenses]) => {
              ...
              return (
                <Box key={month} mb={6}>
                  ...
                  <Stack gap={2}>
                    <For each={expenses}>{(item) => <ExpenseCard key={item.expenseId} item={item} />}</For>
                  </Stack>
                </Box>
              );
            }}
