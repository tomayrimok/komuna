    ...
    return (
        <Card.Root width="100%" >
                        ...
                        <For each={data.balanceDetails}>
                            {(item) => (
                                <Stack key={item.debt_debtId} direction="row" gap="2" alignItems="center">
                                    {item.debtor ?
                                        t('payments.you-owe', { amount: item.debt_amount, name: `${item.userTo_firstName} ${item.userTo_lastName[0]}` }) :
                                        t('payments.owe-you', { amount: item.debt_amount, name: `${item.userFrom_firstName} ${item.userFrom_lastName[0]}` })
                                    }
                                </Stack>
                            )}
                        </For>
            ...
            <Card.Footer justifyContent="flex-end">
                <Button>{t('payments.settle-up')}</Button>
            </Card.Footer>
        </Card.Root>
    )
}
